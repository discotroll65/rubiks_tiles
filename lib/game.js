(function(){
  ...
  Game.prototype.performPositiveXRotate = function(){
    this.xAxisRotate({
      positive: true,
      newCenterPlace: "bottomStart",
      newTopPlace: "centerStart",
      newBackPlace: "topStart",
      newBottomPlace: "backStart"
    });
  };
  ...
  Game.prototype.performNegativeXRotate = function(){
    this.xAxisRotate({
      negative: true,
      newCenterPlace: "topStart",
      newTopPlace: "backStart",
      newBackPlace: "bottomStart",
      newBottomPlace: "centerStart"
    });
  };

  Game.prototype.xAxisRotate = function(opts){
    var oldCenter = this.faces[this.facePerspective.centerFace];
    var oldTop = this.faces[this.facePerspective.topFace];
    var oldBack = this.faces[this.facePerspective.backFace];
    var oldBottom = this.faces[this.facePerspective.bottomFace];
    var leftFace = this.faces[this.facePerspective.leftFace];
    var rightFace = this.faces[this.facePerspective.rightFace];


    oldCenter.reassignWindowPlace(opts.newCenterPlace);
    oldTop.reassignWindowPlace(opts.newTopPlace);
    if(opts.negative){ oldTop.moveRotate().moveRotate(); }
    oldBack.reassignWindowPlace(opts.newBackPlace);
    if(opts.positive){ oldBack.moveRotate().moveRotate(); }
    oldBottom.reassignWindowPlace(opts.newBottomPlace);
    oldBottom.moveRotate().moveRotate();

    this.facePerspective.bottomFace = oldCenter.color + "Face";
    this.facePerspective.centerFace = oldTop.color + "Face";
    this.facePerspective.topFace = oldBack.color + "Face";
    this.facePerspective.backFace = oldBottom.color + "Face";

    (opts.positive) ? leftFace.moveRotate() : leftFace.moveRotateI();
    (opts.positive) ? rightFace.moveRotateI() : rightFace.moveRotate();
    this.draw(this.ctx);
  };
