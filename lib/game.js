(function(){
  ...
  Game.prototype.handleF_Fi_U_UiMoves = function(opts){
    var i;
    var oldShiftingRowOfTop = this.facePerspective
      .topFace.grid[opts.leftAndTopIdxs].map(function(cell){
        return cell.color;
      });
    var oldShiftingColOfLeft = [
      this.facePerspective.leftFace.grid[2][opts.leftAndTopIdxs].color,
      this.facePerspective.leftFace.grid[1][opts.leftAndTopIdxs].color,
      this.facePerspective.leftFace.grid[0][opts.leftAndTopIdxs].color
    ];
    var oldShiftingRowOfBottom = this.facePerspective
      .bottomFace.grid[opts.rightAndBottomIdxs].map(function(cell){
        return cell.color;
      });
    var oldShiftingColOfRight = [
    ...
    if (opts.inverse){
      for( i = 0; i < 3; i++){
        this.facePerspective.topFace.grid[opts.leftAndTopIdxs][i]
          .color = oldShiftingColOfRight.reverse()[i];
        this.facePerspective.leftFace.grid[i][opts.leftAndTopIdxs]
          .color = oldShiftingRowOfTop.reverse()[i];
        this.facePerspective.bottomFace.grid[opts.rightAndBottomIdxs][i]
          .color = oldShiftingColOfLeft.reverse()[i];
        this.facePerspective.rightFace.grid[i][opts.rightAndBottomIdxs]
          .color = oldShiftingRowOfBottom.reverse()[i];
      }
    } else{
      for( i = 0; i < 3; i++){
        this.facePerspective.topFace.grid[opts.leftAndTopIdxs][i]
          .color = oldShiftingColOfLeft[i];
        this.facePerspective.leftFace.grid[i][opts.leftAndTopIdxs]
          .color = oldShiftingRowOfBottom[i];
        this.facePerspective.bottomFace.grid[opts.rightAndBottomIdxs][i]
          .color = oldShiftingColOfRight[i];
        this.facePerspective.rightFace.grid[i][opts.rightAndBottomIdxs]
          .color = oldShiftingRowOfTop[i];
      }
    }
  };
